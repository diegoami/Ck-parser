...
def test_directory_loads_every_snapshot_oldest_first(tmp_path, capsys):
    ...
    held = [ln for ln in out.splitlines() if "'holder': 200" in ln and "'from': datetime.date(1090, 2, 1)" in ln]
    assert "'to': datetime.date(1100, 6, 1)" in held[0] and "'open': True" in held[0]
    assert "'to': datetime.date(1110, 5, 5)" in held[1] and "'open': False" in held[1]
...
def test_changed_death_date_is_reported(tmp_path, capsys):
    tmp_path = _two_snapshots(
        tmp_path, edits=SUCCESSION_EDITS + ((FOUNDER_DEATH, 'dead_data={\n\t\t\tdate=881.1.1'),)
    )
    assert main([str(tmp_path), "--title", "k_testland", "--dry-run"]) == 1
    _, err = capsys.readouterr()
